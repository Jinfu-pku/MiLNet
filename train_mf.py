    ...
    for ep in range(cfg['epochs']):
        ...
        for i, sample in enumerate(train_loader):
            ...
            if cfg['inputs'] == 'rgb':
                ...
                bound_label = sample['bound'].to(device)
                binary_label = sample['binary_label'].to(device)

                # bound_label1 = F.interpolate(bound_label, (120, 160))
                # bound_label2 = F.interpolate(bound_label, (60, 80))
                # binary_label3 = F.interpolate((binary_label, (60, 80)))
                # binary_label4 = F.interpolate((binary_label, (120, 160)))
                ...
                ####### layer 的多尺度监督
                # bound_label1 = sample['bound1'].to(device)
                # bound_label2 = sample['bound2'].to(device)
                # binary_label3 = sample['binary3'].to(device)
                # binary_label4 = sample['binary4'].to(device)
                ...
                # print(type(label), type(s_label1))
                # print(label.shape)
                # print(s_label1.shape)
            ...
            with amp.autocast():
                ...
                # s_flat = F.softmax(student / Temp, dim=1).reshape(student.shape[0], -1)
                # t_flat = F.softmax(teachers_batch / Temp, dim=1).reshape(teachers_batch.shape[0], -1)
                #
                # loss_soft = Temp * Temp * soft_loss(s_flat, t_flat)  # MSE loss
            ...
            # torch.nn.utils.clip_grad_norm(parameters=model.parameters(), max_norm=10, norm_type=2) #####梯度裁剪
            # assert torch.isnan(model.mu).sum() == 0, print(model.mu)
            Scaler.step(optimizer)
            # assert torch.isnan(model.mu).sum() == 0, print(model.mu)
            # assert torch.isnan(model.mu.grad).sum() == 0, print(model.mu.grad) # 考略更换优化器或者 learning rate
            Scaler.update()
        ...
        with torch.no_grad():
            ...
            # running_metrics_test2.reset()
            # test_loss_meter2.reset()
            # for i, sample in enumerate(test_loader2):
            #     if cfg['inputs'] == 'rgb':
            #         image = sample['image'].to(device)
            #         label = sample['label'].to(device)
            ...
            #
            #     loss = criterion(predict, label) # 计算loss是不变的
            #     test_loss_meter2.update(loss.item())
            #
            #     predict = predict.max(1)[1].cpu(  ).numpy()  # [1, h, w] # 预测结构方法不变
            #     label = label.cpu().numpy()
            #     running_metrics_test2.update(label, predict)

        # train_loss = train_loss_meter.avg   # 打印平均损失
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="configs/MiLNet_mfnet.json", help="Configuration file to use")
